...
import { createAgentX } from "@agentx-cli/sdk"
...
`)
  ...
  const agentx = await createAgentX({ port: 0 })
  ...
  try {
    ...
    const session = await agentx.client.session.create()
    const result = await agentx.client.session
      .prompt({
