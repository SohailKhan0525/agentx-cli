...
The agentx server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@agentx-cli/llm` tool loop after replacing its
...
- plugins get global "agentx" instance like in that post i showed
- agentx instance has stuff like `agentx.session.prompt()` or
  `agentx.tool.register({...})`
...
- decide whether to preserve deprecated `@agentx-cli/llm` orchestration exports
