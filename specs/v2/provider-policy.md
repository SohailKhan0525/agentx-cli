...
Both `action` and `resource` use agentx's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.agentx` files is intentionally deferred until `.agentx` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether agentx permits an operation through its managed execution paths.
