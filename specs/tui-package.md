...
Move the canonical AgentX terminal application from
`packages/agentx/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/agentx`,
`packages/cli`, or `@agentx-cli/core`.

The SDK is the TUI's AgentX boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/agentx`.
- Do not replace private `packages/agentx` imports with `@agentx-cli/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@agentx-cli/tui`.
...
- AgentX domain data displayed by the TUI
...
```text
packages/agentx/src/cli/cmd/tui
```

Its private dependency on `packages/agentx` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/agentx/src/*`.
...
Status: Completed. The private `@agentx-cli/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@agentx-cli/tui`.
...
- The package has no dependency on `agentx`, `@agentx-cli/cli`, or
  `@agentx-cli/core`.
...
primitives now live in `@agentx-cli/tui`. The legacy host consumes explicit
...
and build inputs through `@agentx-cli/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads AgentX globals or process state;
...
Replace process-global AgentX state with resolved TUI inputs.
...
`@agentx-cli/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/agentx` initially.
...
canonical in `@agentx-cli/tui`. Configured references resolve through the new
...
adapters rather than AgentX domain implementations.

Make the SDK the only AgentX domain boundary used by the TUI.
...
- Domain-facing TUI code imports AgentX data and operations only from
  `@agentx-cli/sdk`.
...
`@agentx-cli/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/agentx`.
...
- The package has no imports from `packages/agentx`, `packages/cli`, or
  `@agentx-cli/core`.
...
public `@agentx-cli/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/agentx`.
- Keep the legacy embedded worker and server startup in `packages/agentx`.
- Change those adapters to load config, create transport inputs, and call the
  public `@agentx-cli/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/agentx`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@agentx-cli/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@agentx-cli/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for AgentX domain state.
...
cd packages/agentx && bun typecheck
...
rg '@agentx-cli/core|packages/agentx|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/agentx/src packages/agentx/test
```
...
- Build the current-platform `packages/agentx` binary.
