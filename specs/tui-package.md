...
```text
packages/opencode ---\
                      > @agent-qofeno/tui -> @agent-qofeno/sdk
packages/cli --------/
```
...
`packages/cli`, or `@agent-qofeno/core`.
...
- Do not replace private `packages/opencode` imports with `@agent-qofeno/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@agent-qofeno/tui`.

## Ownership Boundary

### `@agent-qofeno/tui` Owns
...
Status: Completed. The private `@agent-qofeno/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@agent-qofeno/tui`.
...
- The package has no dependency on `opencode`, `@agent-qofeno/cli`, or
  `@agent-qofeno/core`.
...
primitives now live in `@agent-qofeno/tui`. The legacy host consumes explicit
...
and build inputs through `@agent-qofeno/tui/runtime`. Movable app, component,
...
`@agent-qofeno/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@agent-qofeno/tui`. Configured references resolve through the new
...
`@agent-qofeno/tui`. The legacy host injects a narrow plugin host that retains
...
public `@agent-qofeno/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@agent-qofeno/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@agent-qofeno/core`, or either
  executable package.
...
rg '@agent-qofeno/core|packages/opencode|packages/cli' packages/tui
