# Remove `packages/agentx/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/agentx/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the agentx legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/agentx/src/storage/db.ts` are concentrated in 22 source files:

- `packages/agentx/src/account/repo.ts`
- `packages/agentx/src/cli/cmd/db.ts`
- `packages/agentx/src/cli/cmd/import.ts`
- `packages/agentx/src/cli/cmd/stats.ts`
- `packages/agentx/src/control-plane/workspace.ts`
- `packages/agentx/src/index.ts`
- `packages/agentx/src/node.ts`
- `packages/agentx/src/permission/index.ts`
- `packages/agentx/src/project/project.ts`
- `packages/agentx/src/server/projectors.ts`
- `packages/agentx/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/agentx/src/server/shared/fence.ts`
- `packages/agentx/src/session/message-v2.ts`
- `packages/agentx/src/session/projectors.ts`
- `packages/agentx/src/session/prompt.ts`
- `packages/agentx/src/session/session.ts`
- `packages/agentx/src/session/todo.ts`
- `packages/agentx/src/share/share-next.ts`
- `packages/agentx/src/storage/db.ts`
- `packages/agentx/src/sync/index.ts`
- `packages/agentx/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy agentx database wrapper; `packages/agentx/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the agentx wrapper.
...
Status: Completed. `SyncEvent` and the agentx projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy agentx database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy agentx database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy agentx database wrapper.
...
All migration groups are complete or superseded. `packages/agentx/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No agentx data-migration group remains.

The previous agentx `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate agentx data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/agentx`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/agentx/src`
- `bun typecheck` from `packages/agentx`
- Relevant package tests from `packages/agentx`, not the repo root
