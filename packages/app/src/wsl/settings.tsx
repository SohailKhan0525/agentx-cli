...
import { Tag } from "@agentx-cli/ui/v2/badge-v2"
...
import { wslAgentXAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.agentxChecks[item.config.distro]
          const agentxAction = () => wslAgentXAction(check())
          const busy = () => wsl.data?.job?.kind === "install-agentx" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={agentxAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installAgentX(item.config.distro))}
                    >
