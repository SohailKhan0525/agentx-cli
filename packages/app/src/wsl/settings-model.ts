...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-agentx" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslAgentXAction(check?: WslAgentXCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install AgentX"
  if (check.matchesDesktop === false) return "Update AgentX"
}
...
export function addServerViewModel(input: {
  ...
  const agentxCheck = selectedDistro ? (state?.agentxChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.agentxChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateAgentX" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.agentxMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installAgentX" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  agentxCheck: WslAgentXCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingAgentX =
    input.state?.job?.kind === "install-agentx" && input.state.job.distro === input.selectedDistro
  if (!ready || probingAgentX) {
    return {
      variant: "contrast",
  ...
  if (!addServerAgentXReady(input.agentxCheck)) {
    const update = !!input.agentxCheck?.resolvedPath && input.agentxCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingAgentX
        ? { key: "wsl.onboarding.updatingAgentX" }
        : update
          ? { key: "wsl.onboarding.updateAgentX" }
          : { key: "wsl.onboarding.installAgentX" },
      disabled: !!input.state?.job || input.adding,
      action: "install-agentx",
      loading: installingAgentX,
...
function addServerAgentXReady(check: WslAgentXCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.agentxChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.agentxChecks[item.name]) return [`agentx:${item.name}`]
    return []
  })
