import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@agent-qofeno/client/promise"
import { Button } from "@agent-qofeno/ui/button"
import { useDialog } from "@agent-qofeno/ui/context/dialog"
import { Dialog } from "@agent-qofeno/ui/dialog"
import { Icon } from "@agent-qofeno/ui/icon"
import { IconButton } from "@agent-qofeno/ui/icon-button"
import { List, type ListRef } from "@agent-qofeno/ui/list"
import { ProviderIcon } from "@agent-qofeno/ui/provider-icon"
import { Spinner } from "@agent-qofeno/ui/spinner"
import { Tag } from "@agent-qofeno/ui/tag"
import { TextField } from "@agent-qofeno/ui/text-field"
import { ButtonV2 } from "@agent-qofeno/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@agent-qofeno/ui/v2/dialog-v2"
import { TextInputV2 } from "@agent-qofeno/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "opencode-go") return language.t("dialog.provider.agentxGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "agentx"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.agentx.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["agentx", "opencode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "agentx" || provider.id === "opencode-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "agentx"
                                ? "dialog.provider.agentx.tagline"
                                : "dialog.provider.agentxGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "agentx"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.agentxZen.line1")}</div>
              <div>{language.t("provider.connect.agentxZen.line2")}</div>
              <div>
                {language.t("provider.connect.agentxZen.visit.prefix")}
                <ExternalLink
                  href="https://github.com/SohailKhan0525/agentx-cli/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.agentxZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.agentxZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "agentx"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.agentxZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.agentxZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.agentxZen.visit.prefix")}
                <ExternalLink href="https://github.com/SohailKhan0525/agentx-cli/zen" tabIndex={-1}>
                  {language.t("provider.connect.agentxZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.agentxZen.visit.suffix")}
              </div>
