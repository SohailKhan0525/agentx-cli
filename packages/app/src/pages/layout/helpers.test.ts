...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("agentx://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("agentx://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("agentx://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("agentx://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("agentx://open-project")).toBeUndefined()
    expect(parseDeepLink("agentx://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "agentx://open-project?directory=/a",
      "agentx://other?directory=/b",
      "agentx://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("agentx://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("agentx://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("agentx://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("agentx://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "agentx://new-session?directory=/a",
      "agentx://open-project?directory=/b",
      "agentx://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __AGENTX__: {
        deepLinks: ["agentx://open-project?directory=/a"],
      },
    } as unknown as Window & { __AGENTX__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["agentx://open-project?directory=/a"])
