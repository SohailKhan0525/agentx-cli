  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://agentx.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://agentx.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://agentx.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://agentx.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://agentx.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://agentx.example.com"),
      ),
    ).toEqual({
