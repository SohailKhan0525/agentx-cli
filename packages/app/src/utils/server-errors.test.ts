...
import type { SessionNotFoundError } from "@agentx-cli/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em agentx.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
