...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@agentx-cli/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/agentx`)).toBe("path")
    expect(inlineCodeKind(`@agentx-cli/app`)).toBe("path")
    expect(inlineCodeKind(`session/status`)).toBe("path")
  })

  test("detects urls", () => {
    expect(inlineCodeKind(`https://github.com/SohailKhan0525/agentx-cli/docs`)).toBe("url")
    expect(inlineCodeKind(`http://localhost:4444`)).toBe("url")
    expect(inlineCodeKind(`file:///tmp/agentx`)).toBeUndefined()
    expect(inlineCodeKind(`ftp://github.com/SohailKhan0525/agentx-cli/docs`)).toBeUndefined()
  })
})
