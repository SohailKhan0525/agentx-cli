...
describe("agentx acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ agentx }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ agentx }))
        ...
        expect(initialized.agentInfo?.name).toBe("AgentX")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ agentx }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ agentx })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("agentx-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "agentx-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.AGENTX_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ agentx }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("agentx-login")
