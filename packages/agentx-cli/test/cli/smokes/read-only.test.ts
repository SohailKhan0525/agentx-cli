...
describe("agentx read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // AGENTX_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ agentx }) =>
      Effect.gen(function* () {
        const r = yield* agentx.spawn(["mcp", "list"])
        agentx.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ agentx }) =>
      Effect.gen(function* () {
        const r = yield* agentx.spawn(["providers", "list"])
        agentx.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ agentx }) =>
      Effect.gen(function* () {
        const r = yield* agentx.spawn(["models"])
        agentx.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ agentx }) =>
      Effect.gen(function* () {
        const r = yield* agentx.spawn(["agent", "list"])
        agentx.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh AGENTX_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ agentx }) =>
      Effect.gen(function* () {
        const r = yield* agentx.spawn(["session", "list"])
        agentx.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ agentx }) =>
      Effect.gen(function* () {
        const r = yield* agentx.spawn(["stats"])
        agentx.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under AGENTX_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ agentx }) =>
      Effect.gen(function* () {
        const r = yield* agentx.spawn(["db", "path"])
        agentx.expectExit(r, 0, "db path")
