...
import type { TuiAttentionSoundPack } from "@agentx-cli/plugin/tui"
...
const { allThemes, addTheme } = await import("@agentx-cli/tui/context/theme")
...
test("does not bootstrap server plugins while initializing tui plugins", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "agentx.json"), JSON.stringify({ plugin: [pathToFileURL(plugin).href] }))
      return { marker }
    },
  })
