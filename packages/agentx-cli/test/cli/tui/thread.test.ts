...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ agentx }) =>
    Effect.gen(function* () {
      const result = yield* agentx.spawn(["--replay-limit", "10"])

      agentx.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ agentx }) =>
    Effect.gen(function* () {
      const result = yield* agentx.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ agentx }) =>
    Effect.gen(function* () {
      const result = yield* agentx.spawn(["--mini", "--port", "4096"])
