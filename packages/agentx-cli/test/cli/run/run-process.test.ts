// Subprocess integration tests for `agentx run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `agentx.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `AGENTX_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("agentx run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const result = yield* agentx.run("say hi")
        agentx.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const result = yield* agentx.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* agentx.run("think", { extraArgs: ["--thinking"] })
        agentx.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* agentx.run("think again")
        agentx.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const result = yield* agentx.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const result = yield* agentx.run("say hi", { format: "json" })
        agentx.expectExit(result, 0)

        const events = agentx.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ agentx }) =>
      Effect.gen(function* () {
        const result = yield* agentx.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const result = yield* agentx.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const result = yield* agentx.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* agentx.run("request permission", { permission: { bash: "ask" } })
        agentx.expectExit(denied, 0)
        ...
        const allowed = yield* agentx.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        agentx.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* agentx.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        agentx.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, agentx }) =>
      Effect.gen(function* () {
        ...
        const server = yield* agentx.serve()

        const result = yield* agentx.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, agentx }) =>
      Effect.gen(function* () {
        const result = yield* agentx.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, agentx }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* agentx.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
