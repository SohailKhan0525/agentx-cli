...
import { AgentXClient, type GlobalEvent } from "@agentx-cli/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<AgentXClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<AgentXClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<AgentXClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<AgentXClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<AgentXClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://agentx.test"),
...
function sdk(
  input: {
    ...
    subscribe?: AgentXClient["event"]["subscribe"]
    globalEvent?: AgentXClient["global"]["event"]
    promptAsync?: AgentXClient["session"]["promptAsync"]
    status?: AgentXClient["session"]["status"]
    messages?: AgentXClient["session"]["messages"]
    children?: AgentXClient["session"]["children"]
    permissions?: AgentXClient["permission"]["list"]
    questions?: AgentXClient["question"]["list"]
  } = {},
) {
  const client = new AgentXClient()

  const subscribe: AgentXClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: AgentXClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: AgentXClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: AgentXClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: AgentXClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: AgentXClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: AgentXClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: AgentXClient["question"]["list"] = input.questions ?? (() => ok([]))
