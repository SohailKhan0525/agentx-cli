...
// terminal sizes. The default agentx tui command is excluded —
// `agentx --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `agentx --help` prints today;
...
// top-level help on `--help` and exits 1; not a real agentx command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("agentx CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ agentx }) =>
      Effect.gen(function* () {
        const topLevel = yield* agentx.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* agentx.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`agentx ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`agentx ${argv.join(" ")} --help`)
        }
