// Subprocess test harness for the agentx CLI. Spawns the real binary against
...
//
// Configuration flows through agentx's built-in test affordances:
//   - AGENTX_CONFIG_CONTENT      : provider config inline, no files to find
//   - AGENTX_TEST_HOME           : pins os.homedir() → tmpdir
//   - AGENTX_DISABLE_PROJECT_CONFIG : skip walking up for agentx.json
//   - AGENTX_PURE                : skip external plugin discovery + install
//   - AGENTX_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `agentx.run` is fully wired. The shape supports adding more
// builders (`agentx.serve(opts)`, `agentx.acp(opts)`, `agentx.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of AgentXCli.
...
import { AppProcess } from "@agentx-cli/core/process"
...
const agentxRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(agentxRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `agentx run`. New flags should
...
// `agentx serve` is a long-lived process — it never exits on its own.
...
// `agentx acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type AgentXCli = {
  ...
  // Spawn `agentx serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `agentx acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("agentx.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          AGENTX_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("agentx.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("agentx.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "agentx server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `agentx serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("agentx.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const agentx: AgentXCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, agentx })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `agentx.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "agentx") {
...
// every call site. Use this for any test that needs the agentx CLI fixture.
//
...
// resources (e.g. `agentx.serve`) without an extra `Effect.scoped` wrapper —
