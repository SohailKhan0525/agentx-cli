...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-agentx-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-agentx-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://github.com/SohailKhan0525/agentx-cli/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-agentx-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-agentx-workspace")
