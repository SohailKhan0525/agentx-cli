...
const original = {
  ...
  envPassword: process.env.AGENTX_SERVER_PASSWORD,
  envUsername: process.env.AGENTX_SERVER_USERNAME,
}
const auth = { username: "agentx", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.AGENTX_SERVER_PASSWORD
  else process.env.AGENTX_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.AGENTX_SERVER_USERNAME
  else process.env.AGENTX_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.AGENTX_SERVER_PASSWORD = auth.password
  Flag.AGENTX_SERVER_USERNAME = auth.username
  process.env.AGENTX_SERVER_PASSWORD = auth.password
  process.env.AGENTX_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-agentx-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-agentx-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-agentx-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.AGENTX_DISABLE_DEFAULT_PLUGINS
    ...
    try {
      ...
      if (previous === undefined) delete process.env.AGENTX_DISABLE_DEFAULT_PLUGINS
      else process.env.AGENTX_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-agentx-ticket": "1" },
      })
