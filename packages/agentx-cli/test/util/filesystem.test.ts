...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "agentx.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "agentx.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "agentx.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "agentx.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["agentx.json", "agentx.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "agentx.json"),
        path.join(project, "agentx.jsonc"),
      ])
    })
  })
