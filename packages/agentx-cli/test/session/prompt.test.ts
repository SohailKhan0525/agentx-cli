...
import { SessionMessageTable } from "@agentx-cli/core/session/sql"
...
import { Shell } from "@agentx-cli/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@agentx-cli/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://github.com/SohailKhan0525/agentx-cli/config.json", ...config }),
  )
})
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("agentx"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
