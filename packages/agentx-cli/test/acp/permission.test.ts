...
import type { Event, AgentXClient } from "@agentx-cli/sdk/v2"
...
type PermissionReplyParams = Parameters<AgentXClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "agentx-acp-permission-"))
