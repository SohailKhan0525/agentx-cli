...
Delete `packages/agentx/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/agentx/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/agentx/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/agentx`
- TUI plugin loader tests in `packages/agentx` if runtime plugin API wiring changed
