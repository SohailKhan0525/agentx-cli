...
Trim `packages/agentx/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/agentx/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/agentx`.
- `[x]` `bun typecheck` from `packages/agentx`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/agentx/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/agentx`.
- `[x]` `bun dev generate > /tmp/agentx-openapi.json` from `packages/agentx`.
...
- Edit `packages/agentx/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/agentx-openapi.json` from `packages/agentx`.
...
- Check source schemas in files like `packages/agentx/src/session/schema.ts`, `packages/agentx/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/agentx/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/agentx/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
