...
- Author package entrypoint is `@agentx-cli/plugin/tui`.
...
```json
{
  "$schema": "https://github.com/SohailKhan0525/agentx-cli/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/agentx-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@agentx-cli/plugin/tui`.
- `@agentx-cli/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@agentx-cli/plugin/tui"
...
- The value is a semver range checked against the running AgentX version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.agentx` is absent, no check is performed (backward compatible).
...
- `agentx plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `agentx plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.agentx` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.agentx`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.agentx`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.agentx` dir or `AGENTX_CONFIG_DIR`, AgentX installs `@agentx-cli/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@agentx-cli/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus AgentX's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
AgentX registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@agentx-cli/plugin/tui`.
...
- `message` is required; `title` defaults to `"agentx"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `agentx.default`.
...
- Local plugins persist installed themes under the local `.agentx/themes` area near the plugin config source.
...
- `--pure` / `AGENTX_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.agentx/plugins/tui-smoke.tsx`
- Local vim plugin: `.agentx/plugins/tui-vim.tsx`
- Local smoke config: `.agentx/tui.json`
- Local smoke theme: `.agentx/plugins/smoke-theme.json`
