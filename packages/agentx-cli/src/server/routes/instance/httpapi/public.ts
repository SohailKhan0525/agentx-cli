...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for agentx serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AgentXHttpApi)
...
export const PublicApi = AgentXHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "agentx",
    version: "1.0.0",
    description: "agentx api",
