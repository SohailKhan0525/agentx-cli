...
import { makeRuntime } from "@agentx-cli/core/effect/runtime"
import semver from "semver"
import { InstallationChannel, InstallationVersion } from "@agentx-cli/core/installation/version"
import { NpmConfig } from "@agentx-cli/core/npm-config"
import { InstallationEvent } from "@agentx-cli/schema/installation-event"
...
const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "SohailKhan0525/tap/agentx"])
      if (tapFormula.includes("agentx")) return "SohailKhan0525/tap/agentx"
    ...
    const upgradeCurl = Effect.fnUntraced(
      function* (target: string) {
        const response = yield* httpOk.execute(HttpClientRequest.get("https://github.com/SohailKhan0525/agentx-cli/install"))
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".agentx", "bin"))) return "curl" as Method
        ...
        for (const check of checks) {
          ...
          const installedName =
            check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "agentx" : "agentx-cli"
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "brew") {
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get("https://formulae.brew.sh/api/formula/agentx.json").pipe(
        ...
        if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              `${yield* NpmConfig.registry(process.cwd())}/@agent-qofeno%2Fagentx-cli/${InstallationChannel}`,
        ...
        if (detectedMethod === "choco") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27agentx%27%20and%20IsLatestVersion&$select=Version",
        ...
        if (detectedMethod === "scoop") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/agentx.json",
        ...
        const response = yield* httpOk.execute(
          HttpClientRequest.get("https://api.github.com/repos/SohailKhan0525/agentx-cli/releases/latest").pipe(
            HttpClientRequest.acceptJson,
          ),
        )
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "brew": {
            ...
            if (formula.includes("/")) {
              const tap = yield* run(["brew", "tap", "SohailKhan0525/tap"], { env })
              if (tap.code !== 0) {
                upgradeResult = tap
                break
              }
              const repo = yield* text(["brew", "--repo", "SohailKhan0525/tap"])
              const dir = repo.trim()
              if (dir) {
