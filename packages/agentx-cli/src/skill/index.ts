...
import { FrontmatterError } from "@agentx-cli/core/v1/config/error"
...
const AGENTX_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with agentx. The model's intuition for what an
// agentx.json should look like is often wrong, and agentx hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch agentx's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_AGENTX_SKILL_NAME = "customize-agentx"
const CUSTOMIZE_AGENTX_SKILL_DESCRIPTION =
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, AGENTX_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@agentx/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_AGENTX_SKILL_NAME] = {
          name: CUSTOMIZE_AGENTX_SKILL_NAME,
