...
import type { LLMClientShape } from "@agentx-cli/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("agentx"))
    return { type: "unsupported", reason: "provider is not openai, agentx, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @agentx-cli/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains agentx-owned. The native runtime only adapts
      // the @agentx-cli/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
