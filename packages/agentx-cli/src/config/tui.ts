...
import { isRecord } from "@agentx-cli/tui/util/record"
...
import { TuiKeybind } from "@agentx-cli/tui/config/keybind"
...
import { TuiConfig } from "@agentx-cli/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@agentx/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old agentx.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.agentx`
  // folders between cwd and home, and AGENTX_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.AGENTX_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit AGENTX_TUI_CONFIG override, if set.
  if (Flag.AGENTX_TUI_CONFIG) {
    const configFile = Flag.AGENTX_TUI_CONFIG
  ...
  // 4. `.agentx` directories (and AGENTX_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".agentx") || dir === Flag.AGENTX_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".agentx") && dir !== Flag.AGENTX_CONFIG_DIR) continue
