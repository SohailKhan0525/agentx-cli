...
import { serviceUse } from "@agentx-cli/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@agentx-cli/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@agentx-cli/core/v1/config/console-state"
...
import { RemoteAuthError } from "@agentx-cli/core/v1/config/error"
import { ConfigPermissionV1 } from "@agentx-cli/core/v1/config/permission"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "agentx.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "agentx.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://github.com/SohailKhan0525/agentx-cli/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/agentx`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://github.com/SohailKhan0525/agentx-cli/config.json"
        ...
        if (Flag.AGENTX_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from AGENTX_CONFIG_DIR", { path: Flag.AGENTX_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.agentx/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (process.env.AGENTX_CONFIG_CONTENT) {
          ...
          yield* Effect.logDebug("loaded custom config from AGENTX_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              ...
              yield* env.set("AGENTX_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.AGENTX_PERMISSION) {
          try {
            ...
            yield* Effect.logWarning("AGENTX_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
