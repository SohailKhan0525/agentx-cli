...
import type { AssistantMessage, Message, AgentXClient, SessionMessageResponse } from "@agentx-cli/sdk/v2"
...
export const AuthMethodID = "agentx-login"
...
export class Service extends Context.Service<Service, Interface>()("@agentx/ACP/Service") {}
...
function makeDirectoryService(sdk: AgentXClient) {
...
function makeUsageService(sdk: AgentXClient) {
...
async function loadDirectorySnapshot(sdk: AgentXClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, agentx provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const agentxProvider = providers[ProviderV2.ID.make("agentx")]
  const agentxModel = agentxProvider ? Provider.sort(Object.values(agentxProvider.models))[0] : undefined
  if (agentxProvider && agentxModel) return { providerID: agentxProvider.id, modelID: agentxModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "AgentX service failure", service })
}
