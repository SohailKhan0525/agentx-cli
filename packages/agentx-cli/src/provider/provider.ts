...
import { Hash } from "@agentx-cli/core/util/hash"
...
import { optional } from "@agentx-cli/core/schema"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@agentx-cli/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    agentx: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["agentx"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from agentx.json provider config
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `agentx/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `agentx auth cloudflare-ai-gateway`.",
        )
      }
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, agentx auth, or provider options.`,
            )
          },
        }
      }
    ...
    local: Effect.fnUntraced(function* (input: Info) {
      return {
        ...
        async discoverModels(): Promise<Record<string, Model>> {
          const models: Record<string, Model> = {}
          const ports = [11434, 1234, 1337] // Ollama, LM Studio, Jan
          
          for (const port of ports) {
            try {
              // Try standard OpenAI models endpoint first
              let res = await fetch(`http://127.0.0.1:${port}/v1/models`, { signal: AbortSignal.timeout(500) })
              let data = await res.json()
              const isOllama = port === 11434
              
              if (isOllama && (!data || !data.data)) {
                res = await fetch(`http://127.0.0.1:${port}/api/tags`, { signal: AbortSignal.timeout(500) })
                data = await res.json()
                data = { data: (data.models || []).map((m: any) => ({ id: m.name })) }
              }
              
              for (const m of data.data || []) {
                const id = m.id
                // Use a different ID to avoid collisions if multiple services expose the same model name
                const uniqueId = `${port}-${id}`
                const systemStr = os.platform() === 'win32' ? 'Windows' : os.platform() === 'darwin' ? 'macOS' : 'Linux'
                const cpu = os.cpus()[0].model
                
                models[uniqueId] = {
                  id: ModelV2.ID.make(uniqueId),
                  providerID: ProviderV2.ID.make("local"),
                  name: `${isOllama ? 'Ollama' : 'Local'} (${id}) on ${systemStr}`,
                  family: "",
                  api: { 
                    id, 
...
export class Service extends Context.Service<Service, Interface>()("@agentx/Provider") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const priority = providerID.startsWith("agentx")
        ? ["gpt-nano"]
