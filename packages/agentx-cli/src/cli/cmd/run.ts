import type { PermissionV1 } from "@agentx-cli/core/v1/permission"
import { FSUtil } from "@agentx-cli/core/fs-util"
// CLI entry point for `agentx run` and `agentx --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`agentx --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`agentx --mini --attach`): connects to a running
//      agentx server and runs interactive mode against it.
//
...
type ModelInput = Parameters<AgentXClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to AGENTX_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: AgentXClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: AgentXClient, sessionID: string) {
      ...
      async function current(sdk: AgentXClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: AgentXClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: AgentXClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: AgentXClient) {
        ...
        async function loop(client: AgentXClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createAgentXClient({
        baseUrl: "http://agentx.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
