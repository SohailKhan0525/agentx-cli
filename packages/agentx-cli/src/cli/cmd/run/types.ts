// Shared type vocabulary for the direct interactive mode (`agentx --mini`).
//
...
import type { AgentXClient, PermissionRequest, QuestionRequest, ToolPart } from "@agentx-cli/sdk/v2"
import type { TuiConfig } from "@agentx-cli/tui/config"
...
type PromptModel = Parameters<AgentXClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<AgentXClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<AgentXClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<AgentXClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<AgentXClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<AgentXClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<AgentXClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<AgentXClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<AgentXClient["question"]["reject"]>[0]
