...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@agentx-cli/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@agentx-cli/core/effect/app-node-builder"
import { errorData, errorMessage } from "@agentx-cli/tui/util/error"
...
import { hasTheme, upsertTheme } from "@agentx-cli/tui/context/theme"
import { Global } from "@agentx-cli/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@agentx-cli/core/util/flock"
import { Flag } from "@agentx-cli/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@agentx-cli/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@agentx-cli/core/v1/config/plugin"
import { createCommandShim } from "@agentx-cli/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@agentx-cli/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".agentx"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".agentx", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".agentx", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.AGENTX_PURE ? [] : pluginOrigins
    if (Flag.AGENTX_PURE && pluginOrigins.length) {
    }
