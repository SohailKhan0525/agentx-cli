...
export const DEPRECATED_PLUGIN_PACKAGES = ["agentx-openai-codex-auth", "agentx-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, agentxVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(agentxVersion) || semver.major(agentxVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.agentx
  if (typeof range !== "string") return
  if (!semver.satisfies(agentxVersion, range)) {
    throw new Error(`Plugin requires agentx ${range} but running ${agentxVersion}`)
  }
}
