...
import { PromptInput } from "@agentx-cli/schema/prompt-input"
import { Session } from "@agentx-cli/schema/session"
import { Project } from "@agentx-cli/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@agentx-cli/schema/schema"
...
import { SessionEvent } from "@agentx-cli/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this AgentX process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this AgentX process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
