...
describe("authorization redirect validation", () => {
  test("allows registered OpenCode callbacks", () => {
    expect(isAllowedAuthorizationRedirect("app", "https://github.com/SohailKhan0525/agentx-cli/auth/callback")).toBe(true)
    expect(isAllowedAuthorizationRedirect("app", "https://github.com/SohailKhan0525/agentx-cli/auth/callback")).toBe(true)
  ...
  test("rejects unregistered clients and external redirects", () => {
    expect(isAllowedAuthorizationRedirect("other", "https://github.com/SohailKhan0525/agentx-cli/auth/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://evil.example/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://github.com/SohailKhan0525/agentx-cli.evil.example/callback")).toBe(false)
