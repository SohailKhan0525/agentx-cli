...
Practical reference for a future `packages/server` split after the agentx
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/agentx`.
...
Until enough shared service code lives outside `packages/agentx`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/agentx`

It should not import `packages/agentx` services while `packages/agentx`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on agentx-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/agentx` implementation details.
