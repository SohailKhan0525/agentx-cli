...
- Author package entrypoint is `@agent-qofeno/plugin/tui`.
...
```json
{
  "$schema": "https://raw.githubusercontent.com/SohailKhan0525/agentx-cli/main/packages/agentx/src/config/tui.schema.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@agent-qofeno/plugin/tui`.
- `@agent-qofeno/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@agent-qofeno/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@agent-qofeno/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@agent-qofeno/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@agent-qofeno/plugin/tui`.
