...
import { createLocalModel, detectLocalProviders, LOCAL_PROVIDERS } from "./local"
...
export namespace Provider {
  ...
  function e2eURL() {
    const url = Env.get("AGENTX_E2E_LLM_URL")
      ...
      local: () =>
        Effect.succeed({
          ...
          async getModel(_sdk: any, modelID: string, options?: Record<string, any>) {
            const baseUrl = options?.baseURL || LOCAL_PROVIDERS.ollama
            return createLocalModel(baseUrl, modelID)
          },
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
          ...
          Effect.gen(function* () {
            ...
            for (const [id, provider] of Object.entries(providers)) {
              ...
              for (const [modelID, model] of Object.entries(provider.models)) {
                ...
                if (model.status === "alpha" && !Flag.AGENTX_ENABLE_EXPERIMENTAL_MODELS)
                  delete provider.models[modelID]
        ...
        const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
          ...
          if (providerID.startsWith("agentx")) {
            priority = ["gpt-5-nano"]
          }
