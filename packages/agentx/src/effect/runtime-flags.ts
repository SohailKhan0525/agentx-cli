...
const bool = (name: string) => {
  const legacy = name.startsWith("AGENTX_") ? name.replace(/^AGENTX_/, "OPENCODE_") : name
  const modern = name.startsWith("OPENCODE_") ? name.replace(/^OPENCODE_/, "AGENTX_") : name
  return Config.boolean(modern).pipe(
    Config.orElse(() => Config.boolean(legacy)),
...
const positiveInteger = (name: string) => {
  const legacy = name.startsWith("AGENTX_") ? name.replace(/^AGENTX_/, "OPENCODE_") : name
  const modern = name.startsWith("OPENCODE_") ? name.replace(/^OPENCODE_/, "AGENTX_") : name
  return Config.number(modern).pipe(
    Config.orElse(() => Config.number(legacy)),
...
const enabledByExperimental = (name: string) => {
  ...
  return Config.all({
    experimental,
    enabled: Config.boolean(modern).pipe(
      Config.orElse(() => Config.boolean(legacy)),
      Config.option,
    ),
...
export class Service extends ConfigService.Service<Service>()("@opencode/RuntimeFlags", {
  ...
  experimentalCodeMode: enabledByExperimental("AGENTX_EXPERIMENTAL_CODE_MODE"),
  ...
  client: Config.string("AGENTX_CLIENT").pipe(
