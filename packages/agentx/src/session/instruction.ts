...
const FILES = [
  "AGENTS.md",
  ...(Flag.AGENTX_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.AGENTX_CONFIG_DIR) {
    files.push(path.join(Flag.AGENTX_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.AGENTX_DISABLE_CLAUDE_CODE_PROMPT) {
...
export namespace Instruction {
  ...
  export class Service extends Context.Service<Service, Interface>()("@agentx/Instruction") {}
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const relative = Effect.fnUntraced(function* (instruction: string) {
          ...
          if (!Flag.AGENTX_CONFIG_DIR) {
            log.warn(
              `Skipping relative instruction "${instruction}" - no AGENTX_CONFIG_DIR set while project config is disabled`,
            )
            return []
          }
          return yield* fs
            .globUp(instruction, Flag.AGENTX_CONFIG_DIR, Flag.AGENTX_CONFIG_DIR)
            .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        })
