...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run agentx",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for agentx session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found agentx session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["agentx", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting agentx...")
        UI.println()

        const agentxArgs = sessionId ? ["-s", sessionId] : []
        const agentxProcess = Process.spawn(["agentx", ...agentxArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await agentxProcess.exited
        if (code !== 0) throw new Error(`agentx exited with code ${code}`)
      },
    })
  },
})
