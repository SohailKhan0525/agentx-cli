...
import { maskKey } from "../../auth/keytar"
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  const candidates = [path.join(baseDir, "agentx.json"), path.join(baseDir, "agentx.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".agentx", "agentx.json"), path.join(baseDir, ".agentx", "agentx.jsonc"))
  }
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        if (entry?.tokens) {
          prompts.log.info(`  Access token: ${maskKey(entry.tokens.accessToken)}`)
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "agentx-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
