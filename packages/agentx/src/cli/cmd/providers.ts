...
import type { Hooks } from "@agentx-cli/plugin"
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/agentx`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "agentx") {
      yield* Prompt.log.info("Create an api key at https://agentx.ai/auth")
    }
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://agentx.ai/docs/providers/#cloudflare-ai-gateway",
      )
    }
