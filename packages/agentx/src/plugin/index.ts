...
export namespace Plugin {
  ...
  export class Service extends Context.Service<Service, Interface>()("@agentx/Plugin") {}
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createAgentxClient({
            ...
            headers: Flag.AGENTX_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.AGENTX_SERVER_USERNAME ?? "agentx"}:${Flag.AGENTX_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.AGENTX_PURE ? [] : (cfg.plugin_origins ?? [])
          if (Flag.AGENTX_PURE && cfg.plugin_origins?.length) {
