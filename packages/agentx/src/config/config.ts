...
import { NamedError } from "@agentx-cli/util/error"
...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/agentx"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "agentx")
      default:
        return "/etc/agentx"
    }
  }
  ...
  export function managedConfigDir() {
    return process.env.AGENTX_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }

  const managedDir = managedConfigDir()

  const MANAGED_PLIST_DOMAIN = "ai.agentx.managed"

  // Keys injected by macOS/MDM into the managed plist that are not AgentX config
  ...
  /**
   * Parse raw JSON (from plutil conversion of a managed plist) into AgentX config.
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.agentx/command/", "/.agentx/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.agentx/agent/", "/.agentx/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: agentx.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for agentx serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://agentx.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://agentx.ai/docs/agents"),
      provider: z
  ...
  export class Service extends Context.Service<Service, Interface>()("@agentx/Config") {}

  function globalConfigFile() {
    const candidates = ["agentx.jsonc", "agentx.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadConfig = Effect.fnUntraced(function* (
          ...
          const normalized = (() => {
            ...
            delete copy.tui
            log.warn("tui keys in agentx config are deprecated; move them to tui.json", { path: source })
            return copy
          })()
          ...
          if (parsed.success) {
            if (!parsed.data.$schema && isFile) {
              parsed.data.$schema = "https://agentx.ai/config.json"
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "agentx.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "agentx.jsonc"))),
          )
            ...
            yield* Effect.promise(() =>
              import(pathToFileURL(legacy).href, { with: { type: "toml" } })
                .then(async (mod) => {
                  ...
                  result["$schema"] = "https://agentx.ai/config.json"
        ...
        const install = Effect.fnUntraced(function* (dir: string) {
          ...
          const plugin = path.join(dir, "node_modules", "@agentx-cli", "plugin", "package.json")
          ...
          const hasDep = json.dependencies?.["@agentx-cli/plugin"] === target
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          const scope = Effect.fnUntraced(function* (source: string) {
            ...
            if (source === "AGENTX_CONFIG_CONTENT") return "local"
          ...
          for (const [key, value] of Object.entries(auth)) {
            if (value.type === "wellknown") {
              ...
              log.debug("fetching remote config", { url: `${url}/.well-known/agentx` })
              const response = yield* Effect.promise(() => fetch(`${url}/.well-known/agentx`))
              if (!response.ok) {
                throw new Error(`failed to fetch remote config from ${url}: ${response.status}`)
              }
              const wellknown = (yield* Effect.promise(() => response.json())) as any
              const remoteConfig = wellknown.config ?? {}
              if (!remoteConfig.$schema) remoteConfig.$schema = "https://agentx.ai/config.json"
              const source = `${url}/.well-known/agentx`
          ...
          if (Flag.AGENTX_CONFIG) {
            yield* merge(Flag.AGENTX_CONFIG, yield* loadFile(Flag.AGENTX_CONFIG))
            log.debug("loaded custom config", { path: Flag.AGENTX_CONFIG })
          }

          if (!Flag.AGENTX_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("agentx", ctx.directory, ctx.worktree),
            )) {
          ...
          if (Flag.AGENTX_CONFIG_DIR) {
            log.debug("loading config from AGENTX_CONFIG_DIR", { path: Flag.AGENTX_CONFIG_DIR })
          }

          const deps: Fiber.Fiber<void, never>[] = []

          for (const dir of unique(directories)) {
            if (dir.endsWith(".agentx") || dir === Flag.AGENTX_CONFIG_DIR) {
              for (const file of ["agentx.json", "agentx.jsonc"]) {
          ...
          if (process.env.AGENTX_CONFIG_CONTENT) {
            const source = "AGENTX_CONFIG_CONTENT"
            const next = yield* loadConfig(process.env.AGENTX_CONFIG_CONTENT, {
              dir: ctx.directory,
              source,
            })
            yield* merge(source, next, "local")
            log.debug("loaded custom config from AGENTX_CONFIG_CONTENT")
          }
          ...
          if (activeOrg) {
            yield* Effect.gen(function* () {
              ...
              if (Option.isSome(tokenOpt)) {
                process.env["AGENTX_CONSOLE_TOKEN"] = tokenOpt.value
                Env.set("AGENTX_CONSOLE_TOKEN", tokenOpt.value)
              }
