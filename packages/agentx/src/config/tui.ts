...
import { TuiKeybind } from "@agent-qofeno/tui/config/keybind"
...
import { TuiConfig } from "@agent-qofeno/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.AGENTX_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== Flag.AGENTX_CONFIG_DIR) continue
