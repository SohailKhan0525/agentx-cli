...
const TUI_SCHEMA_URL = "https://agentx.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from agentx.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const agentx = await agentxFiles(input)
  for (const file of agentx) {
...
async function agentxFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.AGENTX_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("agentx", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "agentx")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "agentx"))
  }
  if (Flag.AGENTX_CONFIG) files.push(Flag.AGENTX_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "agentx"))
