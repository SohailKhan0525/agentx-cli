...
import type { AssistantMessage, Event, AgentxClient, SessionMessageResponse, ToolPart } from "@agentx-cli/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: AgentxClient }) {
    return {
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // AgentX stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const agentxProvider = providers.find((p) => p.id === "agentx")
    if (agentxProvider) {
      if (agentxProvider.models["big-pickle"]) {
        return { providerID: ProviderID.agentx, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(agentxProvider.models))
      if (best) {
        return {
