...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for agentx.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal agentx sessions
...
agentx acp --cwd /path/to/project
```
...
```bash
AGENTX_ENABLE_QUESTION_TOOL=1 agentx acp
```
...
- **Terminal Integration**: Full terminal support via agentx's bash tool
...
ACP sessions map cleanly to agentx's internal session model:
