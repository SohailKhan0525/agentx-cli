...
describe("file.ripgrep", () => {
  test("defaults to include hidden", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "visible.txt"), "hello")
        await fs.mkdir(path.join(dir, ".agentx"), { recursive: true })
        await fs.promises.writeFile(path.join(dir, ".agentx", "thing.json"), "{}")
      },
    })

    const files = await Array.fromAsync(Ripgrep.files({ cwd: tmp.path }))
    const hasVisible = files.includes("visible.txt")
    const hasHidden = files.includes(path.join(".agentx", "thing.json"))
...
describe("Ripgrep.Service", () => {
  test("search returns empty when nothing matches", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "match.ts"), "const value = 'other'\n")
      },
    })
  ...
  test("search returns matched rows", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "match.ts"), "const value = 'needle'\n")
        await fs.promises.writeFile(path.join(dir, "skip.txt"), "const value = 'other'\n")
      },
    })
  ...
  test("files returns stream of filenames", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(path.join(dir, "b.txt"), "world")
      },
    })
  ...
  test("files respects glob filter", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "keep.ts"), "yes")
        await fs.promises.writeFile(path.join(dir, "skip.txt"), "no")
      },
    })
