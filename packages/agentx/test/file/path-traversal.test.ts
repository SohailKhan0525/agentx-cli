...
describe("File.read path traversal protection", () => {
  test("rejects ../ traversal attempting to read /etc/passwd", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "allowed.txt"), "allowed content")
      },
    })
  ...
  test("allows valid paths within project", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "valid.txt"), "valid content")
      },
    })
...
describe("File.list path traversal protection", () => {
  ...
  test("allows valid subdirectory listing", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "subdir", "file.txt"), "content")
      },
    })
...
describe("Instance.containsPath", () => {
  ...
  test("returns true for path inside worktree but outside directory (monorepo subdirectory scenario)", async () => {
    ...
    await Instance.provide({
      directory: subdir,
      fn: () => {
        // .agentx at worktree root, but we're running from packages/lib
        expect(Instance.containsPath(path.join(tmp.path, ".agentx", "state"))).toBe(true)
