...
describe("tool.skill", () => {
  it.live("description lists skill location URL", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          const skill = path.join(dir, ".agentx", "skill", "tool-skill")
...
`,
          ...
          const home = process.env.AGENTX_TEST_HOME
          process.env.AGENTX_TEST_HOME = dir
      ...
      (dir) =>
        Effect.gen(function* () {
          for (const [name, description] of [
            ...
            const skill = path.join(dir, ".agentx", "skill", name)
            yield* Effect.promise(() =>
              fs.promises.writeFile(
                path.join(skill, "SKILL.md"),
                `---
...
`,
  ...
  test("execute returns skill content block with files", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const skillDir = path.join(dir, ".agentx", "skill", "tool-skill")
...
`,
        ...
        await fs.promises.writeFile(path.join(skillDir, "scripts", "demo.txt"), "demo")
      },
    })

    const home = process.env.AGENTX_TEST_HOME
    process.env.AGENTX_TEST_HOME = tmp.path
    ...
    try {
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const dir = path.join(tmp.path, ".agentx", "skill", "tool-skill")
