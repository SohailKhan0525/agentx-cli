...
const shells = (() => {
  ...
  const list = [bash, null, null, process.env.COMSPEC || null]
...
const fill = (mode: "lines" | "bytes", n: number) => {
  const code =
    mode === "lines"
      ? "console.log(Array.from({length:Number(process.argv[1])},(_,i)=>i+1).join(String.fromCharCode(10)))"
      : "process.stdout.write(String.fromCharCode(97).repeat(Number(process.argv[1])))"
...
describe("tool.bash permissions", () => {
      ...
      test(
        ...
        withShell({ label: "bash", shell: bash }, async () => {
          await using outerTmp = await tmpdir({
            init: async (dir) => {
              await fs.promises.writeFile(path.join(dir, "outside.txt"), "x")
            },
          })
          await Instance.provide({
            directory: projectRoot,
            ...
            fn: async () => {
              ...
              await expect(
                Effect.runPromise(
                  bash.execute(
                    {
                      command: "cat /tmp/agentx-does-not-exist",
  ...
  each("does not ask for external_directory permission when rm inside project", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "tmpfile"), "x")
      },
    })
    await Instance.provide({
      directory: tmp.path,
