...
describe("Project.fromDirectory", () => {
  test("should handle git repository with no commits", async () => {
    ...
    const agentxFile = path.join(tmp.path, ".git", "agentx")
    expect(fs.existsSync(agentxFile)).toBe(false)
  })
  ...
  test("should handle git repository with commits", async () => {
    ...
    const agentxFile = path.join(tmp.path, ".git", "agentx")
    expect(fs.existsSync(agentxFile)).toBe(true)
  })
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("worktree should share project ID with main repo", async () => {
    ...
    try {
      ...
      const cache = path.join(tmp.path, ".git", "agentx")
...
describe("Project.discover", () => {
  test("should discover favicon.png in root", async () => {
    ...
    await fs.promises.writeFile(path.join(tmp.path, "favicon.png"), pngData)
  ...
  test("should not discover non-image files", async () => {
    ...
    await fs.promises.writeFile(path.join(tmp.path, "favicon.txt"), "not an image")
