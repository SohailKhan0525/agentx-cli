...
function paid(providers: Awaited<ReturnType<typeof list>>) {
  const item = providers[ProviderID.make("agentx")]
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".agentx", "plugin")
...
test("agentx loader keeps paid models when config apiKey is present", async () => {
...
test("agentx loader keeps paid models when auth exists", async () => {
  await using base = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(
        path.join(dir, "agentx.json"),
