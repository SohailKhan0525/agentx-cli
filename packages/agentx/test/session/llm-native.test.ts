...
import { LLMEvent, ToolFailure } from "@agentx-cli/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@agentx-cli/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("agentx"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("agentx") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("agentx") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, agentx, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored agentx auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("agentx") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("agentx"),
