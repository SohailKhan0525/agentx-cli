...
import { SessionV1 } from "@agentx-cli/core/v1/session"
import { FSUtil } from "@agentx-cli/core/fs-util"
import { ModelsDev } from "@agentx-cli/core/models-dev"
import { HttpRecorder } from "@agentx-cli/http-recorder"
import { HttpRecorderInternal } from "@agentx-cli/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@agentx-cli/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@agentx-cli/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.agentx.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.AGENTX_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["agentx", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AGENTX_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("AGENTX_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["agentx", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "agentx-proxy",
    name: "AgentX proxy",
    providerID: ProviderV2.ID.agentx,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["agentx", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.AGENTX_RECORD_CONSOLE_TOKEN && process.env.AGENTX_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.agentx,
        name: "AgentX Zen",
        env: ["AGENTX_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.AGENTX_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.AGENTX_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.AGENTX_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["agentx", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AGENTX_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("AGENTX_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("AGENTX_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set AGENTX_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the agentx LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://agentx.ai/config.json", ...scenario.config(model) }),
    ),
  )
