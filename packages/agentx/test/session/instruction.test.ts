...
describe("Instruction.resolve", () => {
  test("returns empty when AGENTS.md is at project root (already in systemPaths)", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "AGENTS.md"), "# Root Instructions")
        await fs.promises.writeFile(path.join(dir, "src", "file.ts"), "const x = 1")
      },
    })
  ...
  test("returns AGENTS.md from subdirectory (not in systemPaths)", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "subdir", "AGENTS.md"), "# Subdir Instructions")
        await fs.promises.writeFile(path.join(dir, "subdir", "nested", "file.ts"), "const x = 1")
      },
    })
    await Instance.provide({
      directory: tmp.path,
...
describe("Instruction.system", () => {
  test("loads both project and global AGENTS.md when both exist", async () => {
    ...
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "AGENTS.md"), "# Global Instructions")
      },
    })
    await using projectTmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "AGENTS.md"), "# Project Instructions")
      },
    })
...
describe("Instruction.systemPaths AGENTX_CONFIG_DIR", () => {
  let originalConfigDir: string | undefined

  beforeEach(() => {
    originalConfigDir = process.env["AGENTX_CONFIG_DIR"]
  })
  ...
  test("prefers AGENTX_CONFIG_DIR AGENTS.md over global when both exist", async () => {
    await using profileTmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "AGENTS.md"), "# Profile Instructions")
      },
    })
    await using globalTmp = await tmpdir({
      init: async (dir) => {
    ...
    process.env["AGENTX_CONFIG_DIR"] = profileTmp.path
    const originalGlobalConfig = Global.Path.config
    ;(Global.Path as { config: string }).config = globalTmp.path
  ...
  test("falls back to global AGENTS.md when AGENTX_CONFIG_DIR has no AGENTS.md", async () => {
    await using profileTmp = await tmpdir()
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "AGENTS.md"), "# Global Instructions")
  ...
  test("uses global AGENTS.md when AGENTX_CONFIG_DIR is not set", async () => {
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "AGENTS.md"), "# Global Instructions")
      },
