...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.AGENTX_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.AGENTX_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.AGENTX_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.AGENTX_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
...
describe("session.prompt_async error handling", () => {
  test("prompt_async route has error handler for detached prompt call", async () => {
    const src = await fs.promises.readFile(new URL("../../src/server/instance/session.ts", import.meta.url, "utf8")).text()
