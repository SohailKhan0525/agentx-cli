...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  ...
  if (options?.config) {
    ...
    await Bun.write(path.join(dirpath, "agentx.json"), configContent)
    await Bun.write(path.join(dirpath, "opencode.json"), configContent)
  }
  ...
  return Effect.gen(function* () {
    ...
    if (options?.config) {
      ...
      yield* Effect.promise(async () => {
        await fs.writeFile(path.join(dir, "agentx.json"), configContent)
        await fs.writeFile(path.join(dir, "opencode.json"), configContent)
      })
    }
