...
async function setupStorageDir() {
  ...
  await fs.promises.writeFile(path.join(storageDir, "migration"), "1")
  return storageDir
}

async function writeProject(storageDir: string, project: Record<string, unknown>) {
  await fs.promises.writeFile(path.join(storageDir, "project", `${project.id}.json`), JSON.stringify(project))
}

async function writeSession(storageDir: string, projectID: string, session: Record<string, unknown>) {
  await fs.promises.writeFile(path.join(storageDir, "session", projectID, `${session.id}.json`), JSON.stringify(session))
}
...
describe("JSON to SQLite migration", () => {
  ...
  test("migrates permissions", async () => {
    ...
    await fs.promises.writeFile(path.join(storageDir, "permission", "proj_test123abc.json"), JSON.stringify(permissionData))
  ...
  test("continues when a JSON file is unreadable and records an error", async () => {
    ...
    await fs.promises.writeFile(path.join(storageDir, "project", "broken.json"), "{ invalid json")
  ...
  test("handles mixed corruption and partial validity in one migration run", async () => {
    ...
    await fs.promises.writeFile(path.join(storageDir, "project", "proj_broken.json"), "{ nope")
    ...
    await fs.promises.writeFile(path.join(storageDir, "message", "ses_test456def", "msg_broken.json"), "{ nope")
    await fs.promises.writeFile(
    ...
    await fs.promises.writeFile(path.join(storageDir, "part", "msg_ok", "part_broken.json"), "{ nope")
    ...
    await fs.promises.writeFile(path.join(storageDir, "todo", "ses_broken.json"), "{ nope")
    ...
    await fs.promises.writeFile(path.join(storageDir, "permission", "proj_broken.json"), "{ nope")
    ...
    await fs.promises.writeFile(path.join(storageDir, "session_share", "ses_broken.json"), "{ nope")
