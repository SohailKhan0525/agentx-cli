...
it.instance("plan agent denies edits except .agentx/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".agentx/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".agentx", "skill", "perm-skill")
...
`,
      ...
      process.env.AGENTX_TEST_HOME = test.directory
