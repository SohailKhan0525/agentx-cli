...
describe("plugin.install.task", () => {
  test("writes both server and tui config entries", async () => {
    ...
    const server = await read(path.join(tmp.path, ".agentx", "agentx.jsonc"))
    const tui = await read(path.join(tmp.path, ".agentx", "tui.jsonc"))
  ...
  test("preserves JSONC comments when adding plugins to server and tui config", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const cfg = path.join(tmp.path, ".agentx")
    const server = path.join(cfg, "agentx.jsonc")
...
`,
  ...
  test("preserves JSONC comments when force replacing plugin version", async () => {
    ...
    const cfg = path.join(tmp.path, ".agentx", "agentx.jsonc")
...
`,
  ...
  test("supports resolver target pointing to a file", async () => {
    ...
    await fs.promises.writeFile(file, "export {}")
    const run = createPlugTask(
      {
        mod: "acme@1.2.3",
      },
  ...
  test("does not change configured package version without force", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server"])
    const cfg = path.join(tmp.path, ".agentx", "agentx.json")
    await fs.mkdir(path.dirname(cfg), { recursive: true })
    await fs.promises.writeFile(cfg, JSON.stringify({ plugin: ["acme@1.0.0"] }, null, 2))
  ...
  test("does not change scoped package version without force", async () => {
    ...
    const cfg = path.join(tmp.path, ".agentx", "agentx.json")
    await fs.mkdir(path.dirname(cfg), { recursive: true })
    await fs.promises.writeFile(cfg, JSON.stringify({ plugin: ["@scope/acme@1.0.0"] }, null, 2))
  ...
  test("keeps file plugin entries and still adds npm plugin", async () => {
    ...
    const cfg = path.join(tmp.path, ".agentx", "agentx.json")
    await fs.mkdir(path.dirname(cfg), { recursive: true })
    await fs.promises.writeFile(cfg, JSON.stringify({ plugin: ["file:///tmp/acme.ts"] }, null, 2))
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "agentx.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".agentx", "agentx.jsonc"))).toBe(false)
  })
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".agentx", "agentx.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(worktree, ".agentx", "agentx.jsonc"))).toBe(false)
  })
  ...
  test("writes tui local scope under directory when worktree is root slash", async () => {
    ...
    const ok = await run(ctxRoot(directory))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".agentx", "tui.jsonc"))).toBe(true)
  })
  ...
  test("writes only tui config for tui-only plugins", async () => {
    ...
    const ok = await run(ctx(tmp.path))
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".agentx", "tui.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".agentx", "agentx.jsonc"))).toBe(false)
  })
  ...
  test("writes tui config for oc-themes-only packages", async () => {
    ...
    await fs.promises.writeFile(path.join(target, "themes", "forest.json"), JSON.stringify({ theme: { text: "#fff" } }, null, 2))
    const run = createPlugTask(
      {
        mod: "acme@1.2.3",
      },
  ...
  test("returns false for oc-themes outside plugin directory", async () => {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, ".agentx", "tui.jsonc"))).toBe(false)
    expect(await Filesystem.exists(path.join(tmp.path, ".agentx", "agentx.jsonc"))).toBe(false)
  })
  ...
  test("force replaces version in both server and tui configs", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const server = path.join(tmp.path, ".agentx", "agentx.json")
    const tui = path.join(tmp.path, ".agentx", "tui.json")
    await fs.mkdir(path.dirname(server), { recursive: true })
    await fs.promises.writeFile(server, JSON.stringify({ plugin: ["acme@1.0.0", "other@1.0.0"] }, null, 2))
    await fs.promises.writeFile(tui, JSON.stringify({ plugin: [["acme@1.0.0", { mode: "safe" }], "other@1.0.0"] }, null, 2))
  ...
  test("returns false and keeps config unchanged for invalid JSONC", async () => {
    ...
    await fs.promises.writeFile(cfg, bad)
