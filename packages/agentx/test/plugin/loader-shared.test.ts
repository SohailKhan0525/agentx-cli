...
describe("plugin.loader.shared", () => {
      ...
      init: async (dir) => {
        ...
        await fs.promises.writeFile(
          ...
          [
            ...
            `  await fs.promises.writeFile(${JSON.stringify(mark)}, \"called\")`,
            "  return {}",
            "}",
            "",
          ].join("\n"),
  ...
  test("deduplicates same function exported as default and named", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(mark, "")
        await fs.promises.writeFile(
          file,
          [
            "const run = async () => {",
            `  const text = await fs.promises.readFile(${JSON.stringify(mark, "utf8")}).text().catch(() => \"\")`,
            `  await fs.promises.writeFile(${JSON.stringify(mark)}, text + \"1\")`,
            "  return {}",
            "}",
            "export default run",
  ...
  test("uses only default v1 server plugin when present", async () => {
      ...
      init: async (dir) => {
        ...
        await fs.promises.writeFile(
          ...
          [
            ...
            '  id: "demo.v1-default",',
            "  server: async () => {",
            `    await fs.promises.writeFile(${JSON.stringify(mark)}, "default")`,
            "    return {}",
            "  },",
            "}",
            "export const named = async () => {",
            `  await fs.promises.writeFile(${JSON.stringify(mark)}, "named")`,
            "  return {}",
            "}",
            "",
          ].join("\n"),
    ...
    await load(tmp.path)
    expect(await fs.promises.readFile(tmp.extra.mark, "utf8")).toBe("default")
  })
  ...
  test("rejects v1 file server plugin without id", async () => {
    ...
    await load(tmp.path)
    const called = await fs.promises.readFile(tmp.extra.mark, "utf8")
      .text()
      .then(() => true)
      .catch(() => false)
      ...
      init: async (dir) => {
        ...
        await fs.promises.writeFile(
          ...
          [
            ...
            '  id: "demo.mixed",',
            "  server: async () => {",
            `    await fs.promises.writeFile(${JSON.stringify(mark)}, "server")`,
            "    return {}",
            "  },",
            "  tui: async () => {},",
            "}",
  ...
  test("resolves npm plugin specs with explicit and default versions", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(path.join(acme, "index.js"), "export default { server: async () => ({}) }\n")
        await fs.promises.writeFile(
          path.join(scope, "package.json"),
          JSON.stringify({ name: "scope-plugin", type: "module", main: "./index.js" }, null, 2),
        )
        await fs.promises.writeFile(path.join(scope, "index.js"), "export default { server: async () => ({}) }\n")
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(path.join(mod, "index.js"), 'import "./main-throws.js"\nexport default {}\n')
        await fs.promises.writeFile(path.join(mod, "main-throws.js"), 'throw new Error("main loaded")\n')
        await fs.promises.writeFile(
        ...
        await fs.promises.writeFile(path.join(mod, "tui.js"), "export default {}\n")

        await fs.promises.writeFile(path.join(dir, "agentx.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
    ...
    try {
      await load(tmp.path)
      expect(await fs.promises.readFile(tmp.extra.mark, "utf8")).toBe("called")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(path.join(mod, "index.js"), "export default {}\n")
        await fs.promises.writeFile(
          path.join(outside, "server.js"),
          [
            "export default {",
            "  server: async () => {",
            `    await fs.promises.writeFile(${JSON.stringify(mark)}, "outside")`,
            "    return {}",
            "  },",
            "}",
            "",
        ...
        await fs.promises.writeFile(path.join(dir, "agentx.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("skips legacy codex and copilot auth plugin specs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(
          ...
          JSON.stringify(
            {
              plugin: ["agentx-openai-codex-auth@1.0.0", "agentx-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
    ...
    try {
      ...
      expect(pkgs).not.toContain("agentx-openai-codex-auth@1.0.0")
      expect(pkgs).not.toContain("agentx-copilot-auth@1.0.0")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("skips broken plugin when install fails", async () => {
      ...
      init: async (dir) => {
        ...
        await fs.promises.writeFile(
          ...
          [
            ...
            '  id: "demo.ok",',
            "  server: async () => {",
            `    await fs.promises.writeFile(${JSON.stringify(mark)}, "ok")`,
            "    return {}",
            "  },",
            "}",
            "",
    ...
    try {
      ...
      expect(await fs.promises.readFile(tmp.extra.mark, "utf8")).toBe("ok")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(path.join(dir, "agentx.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(path.join(dir, "agentx.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
      ...
      init: async (dir) => {
        ...
        await fs.promises.writeFile(
          ...
          [
            ...
            `    await fs.promises.writeFile(${JSON.stringify(mark)}, JSON.stringify(options ?? null))`,
            "    return {}",
            "  },",
            "}",
            "export default plugin",
...
`,
        ...
        await fs.promises.writeFile(path.join(dir, "agentx.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.AGENTX_PURE
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.AGENTX_PURE
      } else {
        process.env.AGENTX_PURE = pure
      }
    }
  })
  ...
  test("handles no-entrypoint tui packages via missing callback", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(path.join(mod, "themes", "night.json"), "{}\n")
        return { mod }
      },
    })
  ...
  test("passes package metadata for entrypoint tui plugins", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(path.join(mod, "tui.js"), 'export default { id: "demo", tui: async () => {} }\n')
        await fs.promises.writeFile(path.join(mod, "themes", "night.json"), "{}\n")
        return { mod }
      },
    })
  ...
  test("rejects oc-themes path traversal", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(file, JSON.stringify({ name: "acme", "oc-themes": ["../escape.json"] }, null, 2))
        return { mod, file }
      },
    })
  ...
  test("retries failed file plugins once after wait and keeps order", async () => {
    ...
    const loaded = await PluginLoader.loadExternal({
      ...
      wait: async () => {
        wait += 1
        await fs.promises.writeFile(path.join(tmp.extra.a, "index.ts"), "export default {}\n")
        await fs.promises.writeFile(path.join(tmp.extra.b, "index.ts"), "export default {}\n")
      },
      report: {
  ...
  test("retries file plugins when finish returns undefined", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(file, "export default {}\n")
        return { spec }
      },
    })
