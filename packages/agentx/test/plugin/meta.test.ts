  ...
  test("tracks file plugin loads and changes", async () => {
    await using tmp = await tmpdir<{ file: string }>({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(file, "export default async () => ({})\n")
        return { file }
      },
    })

    process.env.AGENTX_PLUGIN_META_FILE = path.join(tmp.path, "state", "plugin-meta.json")
    const file = process.env.AGENTX_PLUGIN_META_FILE!
    ...
    await fs.promises.writeFile(tmp.extra.file, "export default async () => ({ ok: true })\n")
  ...
  test("tracks npm plugin versions", async () => {
    await using tmp = await tmpdir<{ mod: string; pkg: string }>({
      init: async (dir) => {
        ...
        await fs.promises.writeFile(pkg, JSON.stringify({ name: "acme-plugin", version: "1.0.0" }, null, 2))
        return { mod, pkg }
      },
    })
    ...
    await fs.promises.writeFile(tmp.extra.pkg, JSON.stringify({ name: "acme-plugin", version: "1.1.0" }, null, 2))
