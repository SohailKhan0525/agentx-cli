...
afterEach(async () => {
  delete process.env.AGENTX_CONFIG
  delete process.env.AGENTX_TUI_CONFIG
  await fs.rm(path.join(Global.Path.config, "agentx.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "agentx.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const local = path.join(dir, ".agentx")
      await fs.mkdir(local, { recursive: true })

      await fs.promises.writeFile(
        path.join(Global.Path.config, "agentx.json"),
        JSON.stringify(
          {
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(Global.Path.config, "tui.json"), JSON.stringify({ theme: "global" }, null, 2))
      await fs.promises.writeFile(path.join(dir, "tui.json"), JSON.stringify({ theme: "project" }, null, 2))
      await fs.mkdir(path.join(dir, ".agentx"), { recursive: true })
      await fs.promises.writeFile(
        path.join(dir, ".agentx", "tui.json"),
...
test("migrates tui-specific keys from agentx.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "agentx.json")))
      expect(server.theme).toBeUndefined()
      expect(server.keybinds).toBeUndefined()
      expect(server.tui).toBeUndefined()
      expect(await Filesystem.exists(path.join(tmp.path, "agentx.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when agentx.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "agentx.jsonc.tui-migration.bak"))).toBe(false)
      const source = await Filesystem.readText(path.join(tmp.path, "agentx.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(dir, "agentx.json"), JSON.stringify({ theme: "legacy" }, null, 2))
      await fs.promises.writeFile(path.join(dir, "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "agentx.json.tui-migration.bak"))).toBe(false)
    },
  })
})
...
test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(dir, "agentx.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "agentx.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await TuiConfig.get()
      const backup = await Filesystem.readText(path.join(tmp.path, "agentx.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple agentx.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await fs.promises.writeFile(path.join(dir, "agentx.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await fs.promises.writeFile(path.join(nested, "agentx.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("project config takes precedence over AGENTX_TUI_CONFIG (matches AGENTX_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(dir, "tui.json"), JSON.stringify({ theme: "project", diff_style: "auto" }))
      const custom = path.join(dir, "custom-tui.json")
      await fs.promises.writeFile(custom, JSON.stringify({ theme: "custom", diff_style: "stacked" }))
      process.env.AGENTX_TUI_CONFIG = custom
    },
  })
...
test("merges keybind overrides across precedence layers", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(Global.Path.config, "tui.json"), JSON.stringify({ keybinds: { app_exit: "ctrl+q" } }))
      await fs.promises.writeFile(path.join(dir, "tui.json"), JSON.stringify({ keybinds: { theme_list: "ctrl+k" } }))
    },
  })
...
wintest("keeps explicit input undo overrides on Windows", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(dir, "tui.json"), JSON.stringify({ keybinds: { input_undo: "ctrl+y" } }))
    },
  })
...
wintest("ignores terminal suspend bindings on Windows", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(dir, "tui.json"), JSON.stringify({ keybinds: { terminal_suspend: "alt+z" } }))
    },
  })
...
test("AGENTX_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
      await fs.promises.writeFile(custom, JSON.stringify({ theme: "from-env", diff_style: "stacked" }))
      process.env.AGENTX_TUI_CONFIG = custom
    },
  })
...
test("does not derive tui path from AGENTX_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await fs.promises.writeFile(path.join(customDir, "agentx.json"), JSON.stringify({ model: "test/model" }))
      await fs.promises.writeFile(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.AGENTX_CONFIG = path.join(customDir, "agentx.json")
    },
  })
...
test("applies env and file substitutions in tui.json", async () => {
  ...
  try {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.writeFile(path.join(dir, "keybind.txt"), "ctrl+q")
        await fs.promises.writeFile(
          path.join(dir, "tui.json"),
          JSON.stringify({
...
test("applies file substitutions when first identical token is in a commented line", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(dir, "theme.txt"), "resolved-theme")
      await fs.promises.writeFile(
...
test("loads .agentx/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".agentx"), { recursive: true })
      await fs.promises.writeFile(path.join(dir, ".agentx", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
...
test("gracefully falls back when tui.json has invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.promises.writeFile(path.join(dir, "tui.json"), "{ invalid json }")
      await fs.mkdir(managedConfigDir, { recursive: true })
      await fs.promises.writeFile(path.join(managedConfigDir, "tui.json"), JSON.stringify({ theme: "managed-fallback" }, null, 2))
    },
  })
