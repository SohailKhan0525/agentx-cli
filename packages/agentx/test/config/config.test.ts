...
async function writeManagedSettings(settings: object, filename = "agentx.json") {
...
test("ignores legacy tui keys in agentx config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://agentx.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "agentx.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["AGENTX_CONSOLE_TOKEN"]
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["AGENTX_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["AGENTX_CONSOLE_TOKEN"]
    }
  }
})
...
test("handles file inclusion with replacement tokens", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "included.md"), "const out = await `echo hi`")
      await writeConfig(dir, {
        $schema: "https://agentx.ai/config.json",
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await load()
      expect(config.username).toBe("const out = await `echo hi`")
    },
  })
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "agentx.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .agentx directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentxDir = path.join(dir, ".agentx")
      await fs.mkdir(agentxDir, { recursive: true })
      const agentDir = path.join(agentxDir, "agent")
...
test("loads agents from .agentx/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentxDir = path.join(dir, ".agentx")
      await fs.mkdir(agentxDir, { recursive: true })

      const agentsDir = path.join(agentxDir, "agents")
...
test("loads commands from .agentx/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentxDir = path.join(dir, ".agentx")
      await fs.mkdir(agentxDir, { recursive: true })

      const commandDir = path.join(agentxDir, "command")
...
test("loads commands from .agentx/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentxDir = path.join(dir, ".agentx")
      await fs.mkdir(agentxDir, { recursive: true })

      const commandsDir = path.join(agentxDir, "commands")
...
test("does not try to install dependencies in read-only AGENTX_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.AGENTX_CONFIG_DIR
  process.env.AGENTX_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.AGENTX_CONFIG_DIR
    else process.env.AGENTX_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable AGENTX_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const install = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    const mod = path.join(dir, "node_modules", "@agentx-cli", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@agentx-cli/plugin", version: "1.0.0" }),
    )
  })
...
it.live("dedupes concurrent config dependency installs for the same dir", () =>
  Effect.gen(function* () {
    ...
    const run = spyOn(Npm, "install").mockImplementation(async (d: string) => {
      ...
      const mod = path.join(d, "node_modules", "@agentx-cli", "plugin")
      await fs.mkdir(mod, { recursive: true })
      await Filesystem.write(
        path.join(mod, "package.json"),
        JSON.stringify({ name: "@agentx-cli/plugin", version: "1.0.0" }),
...
it.live("serializes config dependency installs across dirs", () =>
  Effect.gen(function* () {
    ...
    const run = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
      ...
      const mod = path.join(cwd, "node_modules", "@agentx-cli", "plugin")
      await fs.mkdir(mod, { recursive: true })
      await Filesystem.write(
        path.join(mod, "package.json"),
        JSON.stringify({ name: "@agentx-cli/plugin", version: "1.0.0" }),
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "agentx.json"),
        JSON.stringify({ $schema: "https://agentx.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .agentx config
      const projectDir = path.join(dir, "project")
      const agentxDir = path.join(projectDir, ".agentx")
      await fs.mkdir(agentxDir, { recursive: true })
      ...
      // Local .agentx config with different plugins
      await Filesystem.write(
        path.join(agentxDir, "agentx.json"),
        JSON.stringify({
          $schema: "https://agentx.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .agentx config with some overlapping plugins
      await Filesystem.write(
        path.join(agentxDir, "agentx.json"),
        JSON.stringify({
          $schema: "https://agentx.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".agentx")
...
test("local .agentx config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .agentx directory config enables it
      const agentxDir = path.join(dir, ".agentx")
      await fs.mkdir(agentxDir, { recursive: true })
      await Filesystem.write(
        path.join(agentxDir, "agentx.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/agentx")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/agentx")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    expect(await Config.resolvePluginSpec("oh-my-agentx@2.4.3", file)).toBe("oh-my-agentx@2.4.3")
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const agentxDir = path.join(projectDir, ".agentx")
        const pluginDir = path.join(agentxDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("AGENTX_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["AGENTX_DISABLE_PROJECT_CONFIG"]
    process.env["AGENTX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        ...
        process.env["AGENTX_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .agentx/ directories when flag is set", async () => {
    const originalEnv = process.env["AGENTX_DISABLE_PROJECT_CONFIG"]
    process.env["AGENTX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .agentx directory with a command
          const agentxDir = path.join(dir, ".agentx", "command")
          await fs.mkdir(agentxDir, { recursive: true })
          await Filesystem.write(path.join(agentxDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .agentx should NOT be in directories list
          const hasProjectAgentx = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectAgentx).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["AGENTX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["AGENTX_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["AGENTX_CONFIG_DIR"]
      process.env["AGENTX_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["AGENTX_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["AGENTX_CONFIG_DIR"]
      } else {
        process.env["AGENTX_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("AGENTX_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["AGENTX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["AGENTX_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["AGENTX_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["AGENTX_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from AGENTX_CONFIG_DIR, not project
...
describe("AGENTX_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in AGENTX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["AGENTX_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["AGENTX_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://agentx.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["AGENTX_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["AGENTX_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in AGENTX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["AGENTX_CONFIG_CONTENT"]
...
test("parseManagedPlist handles empty config", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({ $schema: "https://agentx.ai/config.json" }),
    "test:mobileconfig",
  )
