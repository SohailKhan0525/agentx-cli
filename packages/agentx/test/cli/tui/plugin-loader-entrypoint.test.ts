...
test("loads npm tui plugin from package ./tui export", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.promises.writeFile(path.join(mod, "server.js"), "export default {}\n")
      await fs.promises.writeFile(
  ...
  tui: async (_api, options) => {
    ...
    await fs.promises.writeFile(${JSON.stringify(marker)}, "called")
  },
}
`,
      )
...
test("rejects npm tui export that resolves outside plugin directory", async () => {
  ...
  id: "demo.outside",
  tui: async () => {
    await fs.promises.writeFile(${JSON.stringify(marker)}, "outside")
  },
}
`,
      )
