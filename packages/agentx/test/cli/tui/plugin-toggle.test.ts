...
test("toggles plugin runtime state by exported id", async () => {
  ...
  tui: async (api, options) => {
    const text = await fs.promises.readFile(options.marker, "utf8").catch(() => "")
    await fs.promises.writeFile(options.marker, text + "start\\n")
    api.lifecycle.onDispose(async () => {
      const next = await fs.promises.readFile(options.marker, "utf8").catch(() => "")
      await fs.promises.writeFile(options.marker, next + "stop\\n")
    })
  },
}
`,
...
test("kv plugin_enabled overrides tui config on startup", async () => {
  ...
  tui: async (_api, options) => {
    await fs.promises.writeFile(options.marker, "on")
  },
}
`,
      )
