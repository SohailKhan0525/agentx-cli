...
async function load(): Promise<Data> {
  ...
  const backup = await fs.promises.readFile(globalConfigPath, "utf8")
    .text()
    .catch(() => undefined)
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".agentx", "themes", localThemeFile)
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".agentx", "themes", preloadedThemeFile)
      ...
      await fs.promises.writeFile(localThemePath, JSON.stringify({ theme: { primary: "#101010" } }, null, 2))
      await fs.promises.writeFile(invalidThemePath, "{ invalid json }")
      await fs.promises.writeFile(globalThemePath, JSON.stringify({ theme: { primary: "#202020" } }, null, 2))
      await fs.promises.writeFile(preloadedThemePath, JSON.stringify({ theme: { primary: "#f0f0f0" } }, null, 2))
      await fs.promises.writeFile(preloadedDest, JSON.stringify({ theme: { primary: "#303030" } }, null, 2))

      await fs.promises.writeFile(
        localPluginPath,
        `export const ignored = async (_input, options) => {
  if (!options?.fn_marker) return
  await fs.promises.writeFile(options.fn_marker, "called")
}
...
export default {
  ...
  tui: async (api, options) => {
    ...
    const first = await fs.promises.readFile(options.dest, "utf8")
    await fs.promises.writeFile(options.source, JSON.stringify({ theme: { primary: "#fefefe" } }, null, 2))
    await api.theme.install(options.theme_path)
    const second = await fs.promises.readFile(options.dest, "utf8")
    await fs.promises.writeFile(
      options.marker,
      JSON.stringify({
        before,
...
`,
  ...
  tui: async (api, options) => {
    ...
    await api.theme.install(options.theme_path)
    const after = api.theme.has(options.theme_name)
    const text = await fs.promises.readFile(options.dest, "utf8")
    await fs.promises.writeFile(
      options.marker,
      JSON.stringify({
        before,
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".agentx", "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
    ...
    if (backup === undefined) {
      ...
      await fs.promises.writeFile(globalConfigPath, backup)
    }
...
test("continues loading when a plugin is missing config metadata", async () => {
  ...
  tui: async (_api, options) => {
    if (!options?.marker) return
    await fs.promises.writeFile(options.marker, "called")
  },
}
`,
        )
  ...
  id: "demo.plain",
  tui: async (_api, options) => {
    await fs.promises.writeFile(${JSON.stringify(bareMarker)}, options === undefined ? "undefined" : "value")
  },
}
`,
      )
  ...
  process.env.AGENTX_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  ...
  try {
    ...
    get.mockRestore()
    wait.mockRestore()
    delete process.env.AGENTX_PLUGIN_META_FILE
  }
})
...
test("initializes external tui plugins in config order", async () => {
  ...
  const backupJson = await fs.promises.readFile(globalJson, "utf8")
    .text()
    .catch(() => undefined)
  const backupJsonc = await fs.promises.readFile(globalJsonc, "utf8")
    .text()
    .catch(() => undefined)
...
`,
      ...
      await fs.promises.writeFile(path.join(dir, "tui.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  try {
    ...
    if (backupJson === undefined) {
      ...
      await fs.promises.writeFile(globalJson, backupJson)
    }
    if (backupJsonc === undefined) {
      await fs.rm(globalJsonc, { force: true }).catch(() => {})
    } else {
      await fs.promises.writeFile(globalJsonc, backupJsonc)
    }
  }
})
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".agentx", "themes", themeFile)
      const themeName = themeFile.replace(/\.json$/, "")
      const configPath = path.join(dir, "tui.json")

      await fs.promises.writeFile(themePath, JSON.stringify({ theme: { primary: "#111111" } }, null, 2))
      await fs.promises.writeFile(
        pluginPath,
        `export default {
...
`,
  ...
  try {
    ...
    await fs.promises.writeFile(tmp.extra.themePath, JSON.stringify({ theme: { primary: "#222222" } }, null, 2))
    await fs.promises.writeFile(
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.AGENTX_PLUGIN_META_FILE!,
    )
