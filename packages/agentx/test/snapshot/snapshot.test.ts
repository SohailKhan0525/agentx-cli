...
test("git info exclude changes", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      await fs.promises.writeFile(file, `${text.trimEnd()}\nignored.txt\n`)
      await fs.promises.writeFile(`${tmp.path}/ignored.txt`, "ignored content")
      await fs.promises.writeFile(`${tmp.path}/normal.txt`, "normal content")
...
test("git info exclude keeps global excludes", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      await fs.promises.writeFile(global, "global.tmp\n")
      ...
      try {
        ...
        await fs.promises.writeFile(`${tmp.path}/global.tmp`, "global content")
        await fs.promises.writeFile(`${tmp.path}/info.tmp`, "info content")
        await fs.promises.writeFile(`${tmp.path}/normal.txt`, "normal content")
