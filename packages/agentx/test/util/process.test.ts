...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.AGENTX_TEST ?? "")'), {
      env: {
        AGENTX_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "AGENTX_TEST_SHELL"], {
      shell: true,
      env: {
        AGENTX_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("AGENTX_TEST_SHELL=ok")
  })
  ...
  test("runs cmd scripts with spaces on Windows without shell", async () => {
    ...
    await fs.promises.writeFile(file, "@echo off\r\nif %~1==--stdio exit /b 0\r\nexit /b 7\r\n")
