...
import { createAgentX, type ToolPart } from "@agentx-cli/sdk"
...
console.log("🚀 Starting agentx server...")
const agentx = await createAgentX({
  port: 0,
})
console.log("✅ AgentX server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await agentx.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new agentx session...")
    const { client, server } = agentx
    ...
    console.log("✅ Created agentx session:", createResult.data.id)
  ...
  console.log("📝 Sending to agentx:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 AgentX response:", JSON.stringify(result, null, 2))
