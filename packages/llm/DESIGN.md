...
> current private `@agentx-cli/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@agentx-cli/ai`
...
- Secondary audience: AgentX and other durable agent runtimes
...
import { LLM } from "@agentx-cli/ai"
import { OpenAI } from "@agentx-cli/ai/providers/openai"
...
```ts
import { Message } from "@agentx-cli/ai"
...
import { LLM, Tool } from "@agentx-cli/ai"
...
AgentX and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@agentx-cli/ai/promise"
import { OpenAI } from "@agentx-cli/ai/providers/openai"
...
```ts
import { LLMSchema } from "@agentx-cli/ai/schema"
...
```ts
import { Provider, Protocol } from "@agentx-cli/ai/provider"
...
@agentx-cli/ai/promise
  Promise/AsyncIterable LLM facade

@agentx-cli/ai/schema
  serializable domain schemas

@agentx-cli/ai/provider
  experimental Provider and Protocol authoring APIs

@agentx-cli/ai/providers/openai
@agentx-cli/ai/providers/anthropic
@agentx-cli/ai/providers/google
...
```
...
AgentX should migrate to `generateTurn` / `streamTurn`, preserving its durable
