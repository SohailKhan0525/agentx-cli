...
import { getAgentXModeStack, AGENTX_BASE_MODE, AgentXKeymapProvider, registerAgentXKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerAgentXKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when agentx mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getAgentXModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getAgentXModeStack(keymap).push("autocomplete")
