...
import {
  ...
  AGENTX_BASE_MODE,
  AgentXKeymapProvider,
  registerAgentXKeymap,
  useBindings,
  useAgentXKeymap,
} from "./keymap"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
      const renderer = yield* Effect.acquireRelease(
        Effect.tryPromise(() =>
          createCliRenderer({
            ...
            useMouse: !Flag.AGENTX_DISABLE_MOUSE && input.config.mouse,
            consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerAgentXKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.AGENTX_ROUTE ? JSON.parse(process.env.AGENTX_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.AGENTX_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
                          <AgentXKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.AGENTX_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.AGENTX_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("AgentX")
      return
    }
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.AGENTX_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://agentx.ai/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.AGENTX_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.AGENTX_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
