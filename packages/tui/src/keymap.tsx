...
const AGENTX_MODE_KEY = "agentx.mode"

export const AgentXKeymapProvider = KeymapProvider
export const useAgentXKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type AgentXModeStack = ReturnType<typeof createAgentXModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, AgentXModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createAgentXModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(AGENTX_MODE_KEY, AGENTX_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(AGENTX_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(AGENTX_MODE_KEY, stack.at(-1)?.mode ?? AGENTX_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? AGENTX_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(AGENTX_MODE_KEY, undefined)
...
export function useAgentXModeStack() {
  return getAgentXModeStack(useAgentXKeymap())
}

export function getAgentXModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("AgentX mode stack is not registered for this keymap")
  return value
}
...
export function registerAgentXKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createAgentXModeStack(keymap)
