...
import type { IntegrationOAuthMethodRegistration } from "@agentx-cli/plugin/v2/effect/integration"
import { define } from "@agentx-cli/plugin/v2/effect/plugin"
import type { CredentialValue } from "@agentx-cli/sdk/v2/types"
...
const defaultServer = "https://console.github.com/SohailKhan0525/agentx-cli"
...
export const AgentXPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "agentx",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("AgentXPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("agentx")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load AgentX provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("agentx", (integration) => {
        integration.name = "AgentX"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "agentx", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("agentx")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("agentx")
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("agentx")),
