...
const reg = (process.env.REGISTRY ?? "ghcr.io/sohailkhan0525").toLowerCase()
...
const setup = async () => {
  ...
  if (list.includes("agentx")) {
    await $`docker buildx use agentx`
    return
  }
  await $`docker buildx create --name agentx --use`
}
