...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve agentx route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "agentx")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "agentx")).toBe("openai")
    expect(statProvider("big-pickle", "", "agentx")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "agentx"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep agentx as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "agentx"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "agentx"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep agentx or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "agentx"), country: "US" })).toMatchObject([
